...
public final class ForwardBlack76 {
    ...
    public double price() {
        ...
        return price(
                contract.type(),
                forwardCurve.forwardPrice(pricingTimestamp),
                contract.strikePrice(),
                fundingCurve.discountFactor(pricingTimestamp),
                volatility,
                timeToExpiry
        );
    }
     ...
     * @return theoretical option value per unit of the underlying
     * @throws IllegalArgumentException if a numeric input is outside its domain,
     * or if {@code timeToExpiry == 0.0} and the discount factor is not one
     */
    public static double price(
        ...
        if (timeToExpiry == 0.0) {
            if (Math.abs(discountFactor - 1.0)
                    > EXPIRY_DISCOUNT_FACTOR_TOLERANCE) {
                throw new IllegalArgumentException(
