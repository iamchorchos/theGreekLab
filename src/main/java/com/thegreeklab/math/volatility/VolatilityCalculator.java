...
public final class VolatilityCalculator {
    ...
    private static final double PRICE_TOLERANCE = 1e-12;
    ...
    public static OptionalDouble impliedVolatility(
        ...
        return solveImpliedVolatility(
                contract, frame, marketPrice, dayCountConvention
        ).asOptionalDouble();
    }

    /**
     * Calculates European implied volatility and returns full solver
     * diagnostics.
     *
     * @param contract the European option contract to calibrate
     * @param frame market data used by the generalized Black-Scholes model
     * @param marketPrice observed option price
     * @param dayCountConvention convention used to derive time to expiry
     * @return diagnostic implied-volatility result
     * @throws NullPointerException if a required object is {@code null}
     * @throws NonPositivePriceException if a required price is not positive
     *                                   and finite
     * @throws UnsupportedExerciseStyleException if the contract is not European
     */
    public static ImpliedVolatilityResult solveImpliedVolatility(
            OptionContract contract,
            MarketData frame,
            double marketPrice,
            DayCountConvention dayCountConvention
        ...
        if (timeToExpiry <= 0) {
            return unavailable(ImpliedVolatilityResult.Status.EXPIRED_CONTRACT);
        }
        ...
        if (marketPrice >= maxPrice) {
            return unavailable(ImpliedVolatilityResult.Status.PRICE_OUTSIDE_BOUNDS);
        }
        ...
        VolatilityPricer model = volatility ->
                BlackScholes.price(contract, frame, volatility, dayCountConvention);
        return solveImpliedVolatility(model, marketPrice, guess);
    }

    /**
     * Solves an implied-volatility problem for any model implementing
     * {@link VolatilityPricer}.
     *
     * <p>The search starts at {@code initialVolatility}, expands geometrically
     * until it brackets a sign change, and then applies Brent's method. Trial
     * values outside a model's numerical domain are skipped while locating a
     * valid bracket.
     *
     * @param model             pricing model evaluated at trial volatilities
     * @param marketPrice       observed price to reproduce
     * @param initialVolatility starting volatility for bracket discovery
     * @return implied volatility, or empty if no root can be bracketed in the
     *         supported volatility interval
     * @throws NullPointerException      if {@code model} is {@code null}
     * @throws NonPositivePriceException if {@code marketPrice} is not strictly
     *                                   positive and finite
     */
    public static OptionalDouble impliedVolatility(
            VolatilityPricer model,
            double marketPrice,
            double initialVolatility
    ) {
        return solveImpliedVolatility(
                model, marketPrice, initialVolatility
        ).asOptionalDouble();
    }

    /**
     * Solves an implied-volatility problem for any volatility-repriceable
     * model and returns full diagnostics.
     *
     * @param model pricing model evaluated at trial volatilities
     * @param marketPrice observed price to reproduce
     * @param initialVolatility starting volatility for bracket discovery
    ...
    public static ImpliedVolatilityResult solveImpliedVolatility(
        ...
        Objects.requireNonNull(model, "Volatility pricer cannot be null.");
        validatePositiveFinitePrice(marketPrice, "Market price");

        if (!Double.isFinite(initialVolatility)
                || initialVolatility < MIN_VOLATILITY_BOUND
                || initialVolatility > MAX_VOLATILITY_BOUND) {
            return unavailable(
        ...
        double error = tryPriceError(model, initialVolatility, marketPrice);
        ...
        if (priceConverged(error, marketPrice)) {
            return result(
        ...
        return error > 0.0
                ? bracketRoot(model, marketPrice, initialVolatility, error,
                        false, iterations)
                : bracketRoot(model, marketPrice, initialVolatility, error,
                        true, iterations);
    }

    private static ImpliedVolatilityResult bracketRoot(
            VolatilityPricer model,
            double marketPrice,
            double anchorVolatility,
            double anchorError,
            boolean searchUp,
            IterationCounter iterations
    ) {
        double validVolatility = anchorVolatility;
        double validError = anchorError;

        for (int i = 0; i < BRENT_MAX_ITERATIONS; i++) {
            iterations.increment();
            double trialVolatility = searchUp
                    ? Math.min(MAX_VOLATILITY_BOUND, validVolatility * 2.0)
                    : Math.max(MIN_VOLATILITY_BOUND, validVolatility * 0.5);

            if (Double.compare(trialVolatility, validVolatility) == 0) {
                return result(
        ...
        return result(
                ImpliedVolatilityResult.Status.MAX_ITERATIONS,
                validVolatility,
                validError,
                marketPrice,
                iterations
    ...
    private static ImpliedVolatilityResult searchValidDomainEdge(
            VolatilityPricer model,
            double marketPrice,
            double validVolatility,
            double validError,
            double invalidVolatility,
            IterationCounter iterations
    ) {
        double valid = validVolatility;
        double invalid = invalidVolatility;
        double edgeError = validError;

        for (int i = 0; i < BRENT_MAX_ITERATIONS; i++) {
            iterations.increment();
            double midpoint = 0.5 * (valid + invalid);
            if (Math.abs(valid - invalid) <= BRENT_TOLERANCE) break;

            double midpointError = tryPriceError(model, midpoint, marketPrice);
            if (Double.isFinite(midpointError)) {
                if (priceConverged(midpointError, marketPrice)) {
                    return result(
                            ImpliedVolatilityResult.Status.CONVERGED,
                            midpoint,
                            midpointError,
                ...
                if (differentSigns(validError, midpointError)) {
                    return brent(model, marketPrice, validVolatility, midpoint,
                            validError, midpointError, BRENT_TOLERANCE,
                            BRENT_MAX_ITERATIONS, iterations);
                }
                valid = midpoint;
                edgeError = midpointError;
            } else {
                invalid = midpoint;
            }
        }

        if (differentSigns(validError, edgeError)) {
            return brent(model, marketPrice, validVolatility, valid,
                    validError, edgeError, BRENT_TOLERANCE,
                    BRENT_MAX_ITERATIONS, iterations);
        }
        return result(
                ImpliedVolatilityResult.Status.INVALID_MODEL_DOMAIN,
    ...
    private static double tryPriceError(
            VolatilityPricer model,
            double volatility,
            double marketPrice
    ) {
        try {
            return model.priceAtVolatility(volatility) - marketPrice;
        } catch (InvalidModelDomainException | InvalidVolatilityException exception) {
            return Double.NaN;
        }
    }

    private static boolean priceConverged(double error, double marketPrice) {
        return Math.abs(error) <= PRICE_TOLERANCE * Math.max(1.0, marketPrice);
    }
    ...
    private static ImpliedVolatilityResult result(
            ImpliedVolatilityResult.Status status,
            double volatility,
            double priceError,
            double marketPrice,
            IterationCounter iterations
    ) {
        double modelPrice = Double.isFinite(priceError)
                ? marketPrice + priceError
                : Double.NaN;
    ...
    private static ImpliedVolatilityResult unavailable(
            ImpliedVolatilityResult.Status status
    ) {
        return new ImpliedVolatilityResult(
                status,
    ...
    private static final class IterationCounter {
        private int value;
    ...
    /**
     * Finds the pricing-error root within the bracket {@code [a, b]}
     ...
     * @param iterations shared total-iteration counter
     * @return diagnostic solver result
     */
    private static ImpliedVolatilityResult brent(
            VolatilityPricer model,
            double marketPrice,
            double a,
            double b,
        ...
        for (int i = 0; i < maxIter; i++) {
            ...
            if (Math.abs(xm) <= tol1 || priceConverged(fb, marketPrice)) {
                return result(
                        ImpliedVolatilityResult.Status.CONVERGED,
                        b,
                        fb,
            ...
            fb = tryPriceError(model, b, marketPrice);
